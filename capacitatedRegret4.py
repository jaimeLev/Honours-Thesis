def zeroRegret(graph, busLoad, capacities):
    ...
    for i in range(len(graph)):
        rowMin = min([j for j in graph[i] if j != 0])
        if rowMin < matrixMin:
            matrixMin = rowMin
    ...
    print(matrixMin, locations)

    # see if the two sides are two endpoints
    # get endpoints which are initially all vertices
    endpoints = []
    ...
    # update the endpoint of the relevant route


    # delete the edge of unimportance and bus if necessary and index of load and distance

    # update distances and loads
